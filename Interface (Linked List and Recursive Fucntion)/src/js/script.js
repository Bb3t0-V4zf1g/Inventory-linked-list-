...
//INSERT PRODUCT IN A SPECIFIC POSITION
const  btnInsert = document.getElementById("btnInsert");
btnInsert.addEventListener("click", (e)=> {
    e.preventDefault();
    let position = document.getElementById("txtPosition").value *1;
    if(!position){
        div.classList.toggle("error", true);
        div.innerHTML = `You have to provide us the number of position`;
    }else{
        //Get name (String)
        let nameP = document.getElementById("txtName").value;
        //Get code (String)
        let codeP = document.getElementById("txtCode").value*1;
        //Get quantity (String)
        let quantityP = document.getElementById("txtQuantity").value * 1; // Change to Int
        //Get price (String)
        let priceP = document.getElementById("txtPrice").value * 1; // Change to Float
        //Instance an object according the proviced datas
        let newP = new Products(nameP, codeP, quantityP, priceP);

        if(!inventory.insert(position, newP)){
            div.classList.toggle("error", true);
            div.innerHTML = `The position is so far than the length of the list or the product already exists`;
        }else{
            div.classList.toggle("error", false);
            //Expose the information
            div.innerHTML = `This product has been added: ${newP.info()}`;
